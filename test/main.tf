...
module "oidc_iam_role" {
  ...
  iam_role_name        = "vcl-oidc-role"
  iam_role_description = "IAM Role for OIDC provider"
  ...
  iam_role_policy_attachment_policy_arns = ["arn:aws:iam::aws:policy/AdministratorAccess"]
...
module "vcl_core_k8s_system_services" {
  source = "../modules/k8s"
  ...
  datadog_agent_confd = {
    "redisdb.yaml"  = <<-EOF
       init_config:
       instances:
         - host: "${module.cache.redis_nodes[0].address}"
           port: 6379
           keys:
            - "*celery*"
    EOF
    "rabbitmq.yaml" = <<-EOF
      init_config:
      instances:
        - rabbitmq_api_url: "${module.rabbitmq.mq_broker_instances[0].console_url}/api/"
          auth_type: "basic"
          username: "mq_broker"
          password: "${var.rabbit_password}"
    EOF
...
module "vcl_workspaces_k8s_system_services" {
  ...
  datadog_api_key                    = var.datadog_api_key
  datadog_network_monitoring_enabled = "true"
  datadog_container_include_logs     = "name:traefik"
  datadog_container_exclude          = "kube_namespace:calico* kube_namespace:tigera*"
  datadog_container_exclude_logs     = "kube_namespace:vcl-system"

  ghost_cpu_enabled  = true
  ghost_cpu_replicas = 0
