...
variable "enable_mq_configuration" {
  description = "Enable MQ configuration"
  default     = false
}
...
variable "mq_broker_deployment_mode" {
  ...
  validation {
    condition     = var.mq_broker_deployment_mode == "SINGLE_INSTANCE" || var.mq_broker_deployment_mode == "CLUSTER_MULTI_AZ"
    error_message = "Only SINGLE_INSTANCE or CLUSTER_MULTI_AZ are allowed."
  }
}
